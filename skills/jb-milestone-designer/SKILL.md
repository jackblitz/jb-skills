...
- **Output**: Native **GitHub Milestones** — one per planned milestone — each with a `Research-[Milestone-Name]` wiki page linked from its description. This skill is the only one that creates GitHub Milestones.
- **Not this skill**: No GitHub Releases, no task issues (that's `jb-task-planner`), no code, no `.md` files in the repo (docs go to the wiki; only `README.md` and `CODING_STANDARDS.md` live in-repo).
...
- **Refine mode**: If the user names an existing milestone (e.g., "Refine Milestone 2"), read it (`.jb/scripts/github-context.sh milestone [name]`) and its research wiki page (`.jb/scripts/github-context.sh research [name]`), then skip to Phase 2 for that milestone only.
...
2. **Research & Discovery**: Before grouping, proactively use autonomous research (e.g., subagents) to understand the technical complexity of the features.
    ...
    - Save each summary to the wiki page `Research-[Milestone-Name]` (pipe into `.jb/scripts/github-wiki.sh put Research-[Milestone-Name]`) to provide a knowledge base for `jb-task-planner` and future agents.
...
2. **Attach Documentation**: Each milestone description MUST contain everything an LLM needs to load context: the Objective, Features, DoD, the target version (e.g., `v0.1.0`), and the link to its `Research-[Milestone-Name]` wiki page (`.jb/scripts/github-wiki.sh url Research-[Milestone-Name]`).
3. **Update the Tracking Issue**: Comment on the `release-plan` issue with the final milestone list and links.

### Phase 4: Anchoring
**Goal**: GitHub Milestones and the wiki are the permanent record of the breakdown.

1. **Sync Milestones**: Ensure every milestone defined in Phase 2 exists in GitHub with the correct description.
2. **Verify Research Pages**: Confirm each milestone's `Research-[Milestone-Name]` wiki page exists (`.jb/scripts/github-wiki.sh list`) and is linked from its description — `jb-task-planner` depends on them.
...
- **No Repo Docs**: Never write research or planning `.md` files into the repo — research lives in the wiki, attached to its milestone by link.
